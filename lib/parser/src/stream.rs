...
impl<'a> Stream<'a> {
    ...
    ///
    /// ```
    /// let s = trax_parser::Stream::from("text");
    ///
    /// assert_eq!(s.gen_text_pos_from(2), trax_parser::TextPos::new(1, 3));
    /// assert_eq!(s.gen_text_pos_from(9999), trax_parser::TextPos::new(1, 5));
    /// ```
    #[inline(never)]
